...
class PlanetTableViewController: UITableViewController {
    ...
    func generatePlanetInfo () {
        ...
        let mars = Planet(name: "Mars", numberOfMoons: 2, fullOrbit: 687.0, facts: ["Mars and Earth have approximately the same landmass.", "Mars is home to the tallest mountain the solar system.", "Pieces of Mars have fallen to Earth."])
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier != "FactCell" {return}
        
        if let destinationVC = segue.destination as? FactTableViewController,
            let indexPath = tableView.indexPathForSelectedRow {
                destinationVC.planet = planets[(indexPath as NSIndexPath).row]
            }
    }
